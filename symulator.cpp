...
void Symulator::set_arx_b(std::vector<double> arx_b) { arx.Set_B(arx_b); }
...
void Symulator::set_syg(signal s) { syg = s; }
...
{
    qDebug() << "[SYMULACJA] trybSieciowy:" << trybSieciowy
             << "siecUAR:" << (siecUAR != nullptr)
             << "polaczono:" << (siecUAR ? siecUAR->polaczono() : false);
    qDebug() << "[SYMULACJA] Wykonywana. Iteracja:" << ite;
    if (syg == signal::sk_jed) u = gen1(ite) - arx.Get_Y();
    else if (syg == signal::syg_prost) u = gen2(ite) - arx.Get_Y();
    else u = gen3(ite) - arx.Get_Y();

    y_pid = pid.symuluj(u); // To jest U

    if (trybSieciowy && siecUAR && siecUAR->polaczono()) {
        qDebug() << "[REGULATOR] Wysyła U:" << y_pid;
        siecUAR->wyslijU(y_pid);
    } else {
        arx.symuluj(y_pid);
    }
...
// odbiór U z sieci po stronie OBIEKTU
void Symulator::odbierzU(double u_z_sieci)
{
    qDebug() << "[OBIEKT] Odebrano U =" << u;
    if (!trybSieciowy) return;

    qDebug() << "[OBIEKT] Otrzymano U:" << u_z_sieci;
...
double Symulator::Get_A_ARX(int numer) { return arx.Get_A(numer); }

double Symulator::Get_B_ARX(int numer) { return arx.Get_B(numer); }

bool Symulator::Get_Z_ARX() { return arx.Get_Z(); }

bool Symulator::Get_K_ARX() { return arx.Get_k(); }

void Symulator::set_stala(double s)
{
    gen1.set_A(s);
    gen2.set_S(s);
    gen3.set_S(s);
...
void Symulator::setSiec(SiecUAR *siec)
{
    siecUAR = siec;
}

void Symulator::setTrybSieciowy(bool stan)
{
    trybSieciowy = stan;
}
